...
4. Use the repository `scripts/start-job-tracker.ps1` for a persistent Windows browser session; that launcher uses port `3217`.
...
Read [references/product-contract.md](references/product-contract.md) before changing fields, classification, grouping, deletion, comparison, reactions, persistence or UI interaction. Treat its layout, copy, focus, hover and confirmation rules as acceptance criteria, not optional suggestions.
...
For progress, batch comparison or decision-history changes, read the
`Application-progress navigation` and `Batch compare and decision history` sections in
the product contract. Verify counts, filters, selection-mode differences, minimum
selection behavior, immutable history snapshots and confirmed history clearing.

When a user reports that an earlier UI decision was forgotten, update the product contract
first, then update the implementation and regression coverage so the decision survives a
fresh Codex task.
